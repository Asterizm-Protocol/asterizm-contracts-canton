...
async function deploy() {
    ...
    }), 'no clientProposalApprovedByRelayerCid') as DamlParty;
    ...
    }), 'no clientCid') as DamlParty;
    console.info('clientCid', clientCid);

    const nftVaultProposalCid = requireValue(await exerciseCmd({
        actAs: [clientOwnerParty],
        payload: {
            templateId: clientTemplateId,
            contractId: clientCid,
            choice: 'CreateNftVaultProposal',
            choiceArgument: {
                dso: DSO,
                vault: vaultParty,
            }
        }
    }), 'no nftVaultProposalCid') as DamlParty;
    console.info('nftVaultProposalCid', nftVaultProposalCid);

    const nftVaultCid = requireValue(await exerciseCmd({
        actAs: [vaultParty],
        payload: {
            templateId: nftVaultProposalTemplateId,
            contractId: nftVaultProposalCid,
            choice: 'NFTVaultProposal_Accept',
            choiceArgument: {},
        }
    }), 'no nftVaultCid') as DamlParty;
    console.info('nftVaultCid', nftVaultCid);
}
